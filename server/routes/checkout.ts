...
// POST /api/checkout/create-trial-session
// 30-minute free trial with card capture.
// Stripe subscription with trial_end = now + 30 min. After trial_end Stripe auto-charges Starter plan.
// User cancels during the 30 minutes to pay nothing. No charge until trial_end passes.
router.post('/create-trial-session', async (req, res) => {
  try {
    const { registrationData } = req.body;

    console.log('🎯 [Trial Checkout] Creating 30-min card-capture trial session');
    ...
    const existingUser = await storage.getUserByEmail(normalizedEmail);
    if (existingUser) {
      console.log(`[Trial Checkout] Blocked: Email ${normalizedEmail} already registered`);
      return res.status(409).json({
        error: 'Email already registered',
        code: 'EMAIL_EXISTS',
        message: 'An account with this email already exists. Please log in or reset your password.',
    ...
    // Reuse or check existing Stripe customer
    let existingStripeCustomerId: string | null = null;
    ...
    try {
      ...
      if (existingCustomers.data.length > 0) {
        ...
        const existingSubs = await stripe.subscriptions.list({ customer: existingCustomer.id, limit: 1 });
        ...
        existingStripeCustomerId = existingCustomer.id;
        console.log(`[Trial Checkout] Reusing existing Stripe customer: ${existingStripeCustomerId}`);
      }
    } catch (stripeError: any) {
      console.error('[Trial Checkout] Stripe customer check failed:', stripeError.message);
    }
    ...
    if (!starterPriceId) {
      return res.status(503).json({
        error: 'Subscription service temporarily unavailable',
        message: 'Starter plan price not configured',
      });
    }
    ...
    // trial_end = exactly 30 minutes from now as Unix timestamp.
    // Stripe subscription_data.trial_end accepts an exact timestamp — this is how we do
    // sub-day trials. trial_period_days only accepts whole days and cannot be used here.
    const trialEndTimestamp = Math.floor(Date.now() / 1000) + (30 * 60);
    ...
    const sessionConfig: any = {
      ...
      metadata: {
        type: 'registration',   // Reuses existing checkout.session.completed webhook handler
        plan: 'starter',
        registrationToken,
        email: registrationData.email,
        isTrial: 'true',        // Flag so webhook sets subscriptionStatus: trialing + 30 min
      },
      subscription_data: {
      ...
      success_url: `${baseUrl}/auth/registration-success?session_id={CHECKOUT_SESSION_ID}&trial=true`,
      cancel_url: `${baseUrl}/start-trial?cancelled=true`,
    ...
    console.log('✅ [Trial Checkout] Session created:', session.id, '| trial_end:', new Date(trialEndTimestamp * 1000).toISOString());
