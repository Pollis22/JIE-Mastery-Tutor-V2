...
import {
  validateWsSession,
  rejectWsUpgrade,
} from "../middleware/ws-session-validator";
...
const TIMING_CONFIG = {
  ...
  SERVER_DELAY_COMPLETE_THOUGHT: 3500, // INCREASED to 4.5s (was 3.5s) - More time for complete sentences
  SERVER_DELAY_INCOMPLETE_THOUGHT: 2000, // INCREASED to 5.5s (was 4.0s) - More time for "um..." and thinking
...
async function persistTranscript(
  ...
  try {
    ...
    console.log(
      `[Custom Voice] 💾 Persisted ${transcript.length} transcript entries`,
    );
  } catch (error) {
...
async function finalizeSession(
  ...
  if (state.isSessionEnded) {
    console.log(
      `[Custom Voice] ℹ️ Session already finalized, skipping (reason: ${reason})`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `[Custom Voice] 💾 Session finalized (${reason}) - ${durationMinutes} minutes, ${state.transcript.length} messages`,
    );
    ...
    if (state.userId && durationMinutes > 0) {
      ...
      console.log(
        `[Custom Voice] ✅ Deducted ${durationMinutes} minutes from user ${state.userId}`,
      );
    }
  } catch (error) {
    console.error(
      `[Custom Voice] ❌ Error finalizing session (${reason}):`,
      error,
    );
  }
}
...
export function setupCustomVoiceWebSocket(server: Server) {
  ...
  console.log(
    "[Custom Voice] WebSocket server initialized on /api/custom-voice-ws (noServer mode)",
  );
  ...
  server.on(
    ...
    async (request: IncomingMessage, socket: Socket, head: Buffer) => {
      ...
      if (!rateLimitCheck.allowed) {
        console.error(
          `[WebSocket] ❌ Rate limit exceeded for ${clientIp}:`,
      ...
      const sessionSecret =
        process.env.SESSION_SECRET || "development-session-secret-only";
      ...
      try {
        wss.handleUpgrade(request, socket, head, (ws) => {
          ...
          if (!trackResult.allowed) {
            console.error(
              `[WebSocket] ❌ Concurrent limit exceeded for ${clientIp}`,
            );
            ws.close(
              1008,
              trackResult.reason || "Too many concurrent connections",
            );
            return;
          }
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    console.log(
      "[Custom Voice] 🔌 New authenticated connection for user:",
      authenticatedUserId,
    );
    ...
    state.inactivityTimerId = setInterval(async () => {
      ...
      console.log(
        `[Inactivity] ⏱️ Check: ${inactiveMinutes}m ${inactiveSeconds}s since last activity`,
      );
      ...
      if (inactiveMinutes >= 4 && !state.inactivityWarningSent) {
        ...
        const warningMessage =
          "Hey, are you still there? I haven't heard from you in a while. If you're done learning for now, just say 'goodbye' or I'll automatically end our session in one minute.";
        ...
        if (state.tutorAudioEnabled) {
          try {
            ...
            console.error(
              "[Inactivity] ❌ Error generating warning audio:",
              audioError,
            );
          }
        }
      ...
      if (inactiveMinutes >= 5) {
        ...
        const endMessage =
          "I haven't heard from you in 5 minutes, so I'm going to end our session now. Feel free to come back anytime you want to learn more!";
        ...
        if (state.tutorAudioEnabled) {
          try {
            ...
            console.error(
              "[Inactivity] ❌ Error generating end audio:",
              audioError,
            );
          }
        }
    ...
    async function processTranscriptQueue() {
      ...
      if (state.isProcessing && processingStartTime) {
        ...
        if (elapsed > MAX_PROCESSING_TIME_MS) {
          console.error(
            `[Pipeline] ⚠️ SAFETY RESET: isProcessing stuck for ${Math.round(elapsed / 1000)}s, forcing reset`,
          );
      ...
      if (
        ...
        if (state.isProcessing && state.transcriptQueue.length > 0) {
          console.log(
            `[Pipeline] 📋 Queue has ${state.transcriptQueue.length} items waiting (isProcessing=true)`,
          );
        }
        return;
      }
      ...
      console.log(
        `[Pipeline] 2. Starting to process transcript, queue size: ${state.transcriptQueue.length + 1}`,
      );
      ...
      try {
        ...
        if (!moderation.isAppropriate) {
          console.log(
            `[Custom Voice] ⚠️  Content flagged: ${moderation.violationType} (confidence: ${moderation.confidence})`,
          );
          ...
          if (moderation.confidence && moderation.confidence > 0.85) {
            console.log(
              "[Custom Voice] ❌ High confidence violation - taking action",
            );
            ...
            if (warningLevel === "none") {
              console.error(
                "[Custom Voice] ❌ Unexpected warning level 'none'",
              );
            ...
            if (warningLevel === "final") {
              console.log(
                "[Custom Voice] 🚫 Suspending user due to repeated violations",
              );
              ...
              await finalizeSession(
                state,
                "violation",
                `Content violation: ${moderation.violationType}`,
              );
              ...
              console.log(
                `[Custom Voice] ⚠️  Sending ${warningLevel} warning to user`,
              );
              const aiResponse = moderationResponse;
            ...
            console.warn(
              "[Custom Voice] ⚠️ Low confidence flag - proceeding with educational context:",
              {
                message: transcript,
                confidence: moderation.confidence,
                reason: moderation.reason,
        ...
        console.log(
          `[Custom Voice] ⏱️ PIPELINE START at ${new Date().toISOString()}`,
        );
        ...
        if (isLikelyIncompleteThought(transcript)) {
          ...
          console.log(
            `[Custom Voice] ⏱️ Detected incomplete thought - using delay (${responseDelay}ms)`,
          );
        } else {
          console.log(
            `[Custom Voice] ⏱️ Complete thought detected - using minimal delay (${responseDelay}ms)`,
          );
        }
        ...
        if (state.wasInterrupted) {
          ...
          if (timeSinceInterrupt < 10000) {
            // Within 10 seconds
            const extraBuffer = 500; // Reduced from 2500ms to 500ms
            console.log(
              `[Custom Voice] 🛑 Post-interruption buffer: +${extraBuffer}ms (interrupted ${timeSinceInterrupt}ms ago)`,
            );
        ...
        console.log(
          `[Custom Voice] ⏳ Pre-response delay: ${responseDelay}ms...`,
        );
        await new Promise((resolve) => setTimeout(resolve, responseDelay));
        console.log(
          `[Pipeline] 3. Calling Claude API with: "${transcript.substring(0, 100)}${transcript.length > 100 ? "..." : ""}"`,
        );
        ...
        console.log(
          `[Custom Voice] 🌍 Generating STREAMING response in: ${responseLanguage}`,
        );
          ...
          const callbacks: StreamingCallbacks = {
            onSentence: async (sentence: string) => {
              ...
              if (sentenceCount === 1) {
                ...
                console.log(
                  `[Custom Voice] ⏱️ First sentence in ${firstSentenceMs}ms`,
                );
              ...
              if (state.tutorAudioEnabled) {
                ...
                try {
                  ...
                  console.log(
                    `[Custom Voice] 🔊 Sentence ${sentenceCount} TTS: ${ttsMs}ms, ${audioBuffer.length} bytes`,
                  );
                  ...
                  console.error(
                    `[Custom Voice] ❌ TTS error for sentence ${sentenceCount}:`,
                    ttsError,
                  );
                }
              }
            ...
            onComplete: (fullText: string) => {
              ...
              console.log(
                `[Pipeline] 4. Claude response received (${claudeMs}ms), generating audio...`,
              );
              ...
              console.log(
                `[Custom Voice] ⏱️ PIPELINE COMPLETE (STREAMING): ${totalPipelineMs}ms total`,
              );
              console.log(
                `[Custom Voice] ⏱️ Breakdown: delay=${responseDelay}ms, firstSentence=${firstSentenceMs}ms, totalTTS=${totalTtsMs}ms, audio=${totalAudioBytes} bytes`,
              );
        ...
        console.log(
          "[Custom Voice] 🔊 Streaming response sent, waiting for user...",
        );
        ...
        console.log(
          `[Custom Voice] ⏳ Pausing ${pauseMs}ms (audio: ${audioDuration.toFixed(1)}s + 1.5s buffer)...`,
        );
        ...
        if (state.lastAudioSentAt === turnTimestamp) {
          ...
          console.log(
            "[Custom Voice] ℹ️ Turn superseded by newer turn, keeping isTutorSpeaking",
          );
        }
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    async function reconnectDeepgram(): Promise<DeepgramConnection> {
      ...
      if (state.deepgramConnection) {
        console.log(
          "[Custom Voice] 🧹 Tearing down old Deepgram connection before reconnect",
        );
        try {
      ...
      const { getDeepgramLanguageCode } = await import(
      ...
      const handleTranscript = async (
        ...
        console.log(
          `[Deepgram] ${isFinal ? "✅ FINAL" : "⏳ interim"}: "${transcript}" (reconnected, lang=${spokenLang})`,
        );
        ...
        // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        console.log(
          `[Pipeline] 1. Transcript received (reconnected): "${transcript}", isProcessing=${state.isProcessing}`,
        );
        ...
        console.log(
          `[Custom Voice] 📝 Accumulated transcript (reconnected): "${pendingTranscript}"`,
        );
        ...
        transcriptAccumulationTimer = setTimeout(() => {
          if (pendingTranscript && !state.isSessionEnded) {
            ...
            console.log(
              `[Custom Voice] ✅ Utterance complete (reconnected): "${completeUtterance}"`,
            );
            pendingTranscript = "";
      ...
      return await startDeepgramStream(
        ...
        async (error: Error) => {
          console.error(
            "[Custom Voice] ❌ Deepgram error (reconnected):",
            error,
          );
          if (state.sessionId && state.transcript.length > 0) {
            await persistTranscript(state.sessionId, state.transcript);
        ...
        async () => {
          console.log(
            "[Custom Voice] 🔌 Reconnected Deepgram connection closed",
          );
    ...
    ws.on("message", async (data: Buffer) => {
      try {
        ...
        switch (message.type) {
          case "init":
            console.log(
              "[Custom Voice] 🚀 Initializing session:",
              message.sessionId,
            );
            ...
            if (message.userId && message.userId !== authenticatedUserId) {
              console.warn(
                `[Custom Voice] ⚠️ Client userId mismatch (ignoring client value)`,
                {
            ...
            try {
              ...
              if (session.length === 0) {
                console.error(
                  `[Custom Voice] ❌ Session not found: ${message.sessionId}`,
                );
                ws.send(
                  JSON.stringify({
                    type: "error",
              ...
              if (session[0].userId !== authenticatedUserId) {
                console.error(
                  `[Custom Voice] ❌ Session ${message.sessionId} does not belong to authenticated user`,
                  {
              ...
              console.log(
                `[Custom Voice] ✅ Session validated for authenticated user ${authenticatedUserId}`,
              );
              ...
              console.error(
                "[Custom Voice] ❌ Session validation error:",
                error,
              );
              ws.send(
                JSON.stringify({
            ...
            try {
              ...
              if (user && user.speechSpeed) {
                state.speechSpeed =
                  typeof user.speechSpeed === "string"
                    ? parseFloat(user.speechSpeed)
                    : user.speechSpeed;
                console.log(
                  `[Custom Voice] ⚙️ User's speech speed preference: ${state.speechSpeed}`,
                );
              } else {
                state.speechSpeed = 0.95; // Default
                console.log(
                  `[Custom Voice] ⚙️ Using default speech speed: 0.95`,
                );
              }
            } catch (error) {
              console.error(
                "[Custom Voice] ⚠️ Error fetching user settings, using default speech speed:",
                error,
              );
            ...
            console.log(
              `[Custom Voice] 🎭 Using personality: ${personality.name} for ${state.ageGroup}`,
            );
            ...
            try {
              ...
              if (
                ...
                typeof messageDocuments[0] === "string" &&
                messageDocuments[0].startsWith("[Document:")
              ) {
                // Frontend has already loaded and sent document content
                console.log(
                  `[Custom Voice] 📚 Received ${messageDocuments.length} pre-loaded documents from frontend`,
                );
                state.uploadedDocuments = messageDocuments;
                const totalChars = messageDocuments.join("").length;
                console.log(
                  `[Custom Voice] 📄 Document context ready: ${messageDocuments.length} documents, total length: ${totalChars} chars`,
                );
              }
              // Otherwise, treat them as document IDs to load from database
                ...
                if (documentIds.length === 0) {
                  console.log(
                    `[Custom Voice] 📄 No specific documents provided, loading all user documents from database...`,
                  );
                  const allUserDocs =
                    await storage.getUserDocuments(authenticatedUserId);
                  const readyDocs = allUserDocs.filter(
                    (doc) => doc.processingStatus === "ready",
                  );
                  documentIds = readyDocs.map((doc) => doc.id);
                  console.log(
                    `[Custom Voice] 📚 Found ${readyDocs.length} ready documents for user`,
                  );
                }

                if (documentIds.length > 0) {
                  console.log(
                    `[Custom Voice] 📄 Loading ${documentIds.length} documents from database...`,
                  );
                  const { chunks, documents } =
                    await storage.getDocumentContext(
                      authenticatedUserId,
                      documentIds,
                    );
                  console.log(
                    `[Custom Voice] ✅ Loaded ${chunks.length} chunks from ${documents.length} documents`,
                  );
                  ...
                  console.log(
                    `[Custom Voice] 📚 Document context prepared: ${documentContents.length} documents, total length: ${documentContents.join("").length} chars`,
                  );
                } else {
                  state.uploadedDocuments = [];
                  console.log(
                    `[Custom Voice] ℹ️ No documents available for this user`,
                  );
                }
              }
            } catch (error) {
              console.error(
                "[Custom Voice] ❌ Error loading documents:",
                error,
              );
              state.uploadedDocuments = [];
            }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
              ...
              console.log(
                `[Custom Voice] 📚 System instruction enhanced with ${state.uploadedDocuments.length} documents`,
              );
            } else {
              // Use standard personality prompt when no documents
              state.systemInstruction =
                personality.systemPrompt + VOICE_CONVERSATION_CONSTRAINTS;
            }
            ...
            const getLocalizedGreeting = (
              ...
              const greetings: Record<
                ...
                {
                  intro: string;
                  docAck: (count: number, titles: string) => string;
                ...
                en: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ...
                      : ` I can see you've uploaded ${count} documents: ${titles}. Great!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Let's look at it together! What do you want to learn about?"
                        : " I'm so excited to learn with you today! What would you like to explore?",
                    "3-5":
                      docTitles.length > 0
                        ? " I'm here to help you understand it! What part should we start with?"
                        : " I'm here to help you learn something new! What subject interests you today?",
                    "6-8":
                      docTitles.length > 0
                        ? " I'm ready to help you master this material! What would you like to work on?"
                        : " I'm here to help you succeed! What subject would you like to focus on today?",
                    "9-12":
                      docTitles.length > 0
                        ? " Let's dive into this material together. What concepts would you like to explore?"
                        : " I'm here to help you excel! What topic would you like to work on today?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " I'm ready to help you analyze this material. What aspects would you like to focus on?"
                        : " I'm here to support your learning goals. What subject can I help you with today?",
                  },
                },
                fr: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Je vois que tu as téléchargé "${titles}" - excellent!`
                      : ` Je vois que tu as téléchargé ${count} documents: ${titles}. Super!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Regardons ça ensemble! Qu'est-ce que tu veux apprendre?"
                        : " Je suis tellement content d'apprendre avec toi! Qu'est-ce qui t'intéresse?",
                    "3-5":
                      docTitles.length > 0
                        ? " Je suis là pour t'aider à comprendre! Par quoi veux-tu commencer?"
                        : " Je suis là pour t'aider à apprendre! Quel sujet t'intéresse?",
                    "6-8":
                      docTitles.length > 0
                        ? " Je suis prêt à t'aider à maîtriser ce contenu! Sur quoi veux-tu travailler?"
                        : " Je suis là pour t'aider à réussir! Sur quel sujet veux-tu travailler?",
                    "9-12":
                      docTitles.length > 0
                        ? " Explorons ce contenu ensemble. Quels concepts voudrais-tu approfondir?"
                        : " Je suis là pour t'aider à exceller! Sur quel sujet voudrais-tu travailler?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Je suis prêt à t'aider à analyser ce contenu. Quels aspects voudrais-tu approfondir?"
                        : " Je suis là pour soutenir tes objectifs d'apprentissage. Comment puis-je t'aider?",
                  },
                },
                es: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Veo que has subido "${titles}" - ¡excelente!`
                      : ` Veo que has subido ${count} documentos: ${titles}. ¡Genial!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " ¡Veámoslo juntos! ¿Qué quieres aprender?"
                        : " ¡Estoy muy emocionado de aprender contigo! ¿Qué te gustaría explorar?",
                    "3-5":
                      docTitles.length > 0
                        ? " ¡Estoy aquí para ayudarte a entender! ¿Por dónde empezamos?"
                        : " ¡Estoy aquí para ayudarte a aprender! ¿Qué tema te interesa?",
                    "6-8":
                      docTitles.length > 0
                        ? " ¡Estoy listo para ayudarte a dominar este material! ¿En qué quieres trabajar?"
                        : " ¡Estoy aquí para ayudarte a tener éxito! ¿En qué tema quieres enfocarte?",
                    "9-12":
                      docTitles.length > 0
                        ? " Exploremos este material juntos. ¿Qué conceptos te gustaría profundizar?"
                        : " ¡Estoy aquí para ayudarte a sobresalir! ¿En qué tema quieres trabajar?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Estoy listo para ayudarte a analizar este material. ¿Qué aspectos te gustaría explorar?"
                        : " Estoy aquí para apoyar tus metas de aprendizaje. ¿Cómo puedo ayudarte?",
                  },
                },
                sw: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Naona umepakia "${titles}" - bora!`
                      : ` Naona umepakia nyaraka ${count}: ${titles}. Vizuri!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Tuangalie pamoja! Unataka kujifunza nini?"
                        : " Ninafuraha sana kujifunza nawe! Unataka kuchunguza nini?",
                    "3-5":
                      docTitles.length > 0
                        ? " Niko hapa kukusaidia kuelewa! Tuanze wapi?"
                        : " Niko hapa kukusaidia kujifunza! Somo gani linakuvutia?",
                    "6-8":
                      docTitles.length > 0
                        ? " Niko tayari kukusaidia kuelewa maudhui haya! Unataka kufanyia kazi nini?"
                        : " Niko hapa kukusaidia kufanikiwa! Unataka kuzingatia somo gani?",
                    "9-12":
                      docTitles.length > 0
                        ? " Tuchunguze maudhui haya pamoja. Dhana gani ungependa kuelewa zaidi?"
                        : " Niko hapa kukusaidia kufanya vizuri! Unataka kufanyia kazi mada gani?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Niko tayari kukusaidia kuchambua maudhui haya. Ungependa kuzingatia vipengele gani?"
                        : " Niko hapa kusaidia malengo yako ya kujifunza. Naweza kukusaidia vipi?",
                  },
                },
                yo: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Mo ri pe o ti fi "${titles}" soke - o dara!`
                      : ` Mo ri pe o ti fi iwe ${count} soke: ${titles}. O dara pupo!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Jẹ ki a wo papọ! Kini o fẹ lati kọ?"
                        : " Mo dun pupọ lati kọ pẹlu rẹ! Kini o fẹ lati ṣawari?",
                    "3-5":
                      docTitles.length > 0
                        ? " Mo wa nibi lati ran ọ lọwọ lati loye! Nibo ni a yoo bẹrẹ?"
                        : " Mo wa nibi lati ran ọ lọwọ lati kọ! Koko-ọrọ wo ni o nifẹ si?",
                    "6-8":
                      docTitles.length > 0
                        ? " Mo ti setan lati ran ọ lọwọ pẹlu ohun elo yii! Kini o fẹ lati ṣiṣẹ lori?"
                        : " Mo wa nibi lati ran ọ lọwọ lati ṣaṣeyọri! Koko-ọrọ wo ni o fẹ dojukọ?",
                    "9-12":
                      docTitles.length > 0
                        ? " Jẹ ki a ṣawari ohun elo yii papọ. Awọn ero wo ni o fẹ jinlẹ?"
                        : " Mo wa nibi lati ran ọ lọwọ lati tayọ! Koko-ọrọ wo ni o fẹ lati ṣiṣẹ lori?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Mo ti setan lati ran ọ lọwọ lati ṣe itupalẹ ohun elo yii. Awọn abala wo ni o fẹ ṣawari?"
                        : " Mo wa nibi lati ṣe atilẹyin awọn ibi-afẹde ẹkọ rẹ. Bawo ni mo ṣe le ran ọ lọwọ?",
                  },
                },
                ha: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Na ga cewa ka loda "${titles}" - kyau!`
                      : ` Na ga cewa ka loda takardun ${count}: ${titles}. Da kyau!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Bari mu duba tare! Mene ne kake so ka koya?"
                        : " Ina farin ciki sosai in koya tare da kai! Mene ne kake so ka bincika?",
                    "3-5":
                      docTitles.length > 0
                        ? " Ina nan don in taimake ka ka fahimta! Ina za mu fara?"
                        : " Ina nan don in taimake ka ka koya! Wane batu ya sha'awar ka?",
                    "6-8":
                      docTitles.length > 0
                        ? " Na shirya in taimake ka da wannan aiki! Mene ne kake so ka yi aiki a kai?"
                        : " Ina nan don in taimake ka ka yi nasara! Wane batu kake so ka mayar da hankali a kai?",
                    "9-12":
                      docTitles.length > 0
                        ? " Bari mu bincika wannan aiki tare. Wane ra'ayoyi kake so ka fahimta sosai?"
                        : " Ina nan don in taimake ka ka yi fice! Wane batu kake so ka yi aiki a kai?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Na shirya in taimake ka ka nazari wannan aiki. Wane fannoni kake so ka bincika?"
                        : " Ina nan don in goyi bayan burin ilimi naka. Ta yaya zan taimake ka?",
                  },
                },
                ar: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` أرى أنك رفعت "${titles}" - ممتاز!`
                      : ` أرى أنك رفعت ${count} مستندات: ${titles}. رائع!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " لنلقي نظرة معاً! ماذا تريد أن تتعلم؟"
                        : " أنا متحمس جداً للتعلم معك! ماذا تريد أن تستكشف؟",
                    "3-5":
                      docTitles.length > 0
                        ? " أنا هنا لمساعدتك على الفهم! من أين نبدأ؟"
                        : " أنا هنا لمساعدتك على التعلم! أي موضوع يثير اهتمامك؟",
                    "6-8":
                      docTitles.length > 0
                        ? " أنا مستعد لمساعدتك في إتقان هذا المحتوى! ما الذي تريد العمل عليه؟"
                        : " أنا هنا لمساعدتك على النجاح! أي موضوع تريد التركيز عليه؟",
                    "9-12":
                      docTitles.length > 0
                        ? " لنستكشف هذا المحتوى معاً. أي مفاهيم تريد التعمق فيها؟"
                        : " أنا هنا لمساعدتك على التفوق! أي موضوع تريد العمل عليه؟",
                    "College/Adult":
                      docTitles.length > 0
                        ? " أنا مستعد لمساعدتك في تحليل هذا المحتوى. أي جوانب تريد استكشافها؟"
                        : " أنا هنا لدعم أهدافك التعليمية. كيف يمكنني مساعدتك؟",
                  },
                },
                de: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Ich sehe, dass du "${titles}" hochgeladen hast - ausgezeichnet!`
                      : ` Ich sehe, dass du ${count} Dokumente hochgeladen hast: ${titles}. Toll!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Lass uns das zusammen ansehen! Was möchtest du lernen?"
                        : " Ich freue mich so, mit dir zu lernen! Was möchtest du erkunden?",
                    "3-5":
                      docTitles.length > 0
                        ? " Ich bin hier, um dir zu helfen es zu verstehen! Womit fangen wir an?"
                        : " Ich bin hier, um dir beim Lernen zu helfen! Welches Thema interessiert dich?",
                    "6-8":
                      docTitles.length > 0
                        ? " Ich bin bereit, dir bei diesem Material zu helfen! Woran möchtest du arbeiten?"
                        : " Ich bin hier, um dir zum Erfolg zu helfen! Auf welches Thema möchtest du dich konzentrieren?",
                    "9-12":
                      docTitles.length > 0
                        ? " Lass uns dieses Material zusammen erkunden. Welche Konzepte möchtest du vertiefen?"
                        : " Ich bin hier, um dir zu helfen, dich auszuzeichnen! An welchem Thema möchtest du arbeiten?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Ich bin bereit, dir bei der Analyse dieses Materials zu helfen. Welche Aspekte möchtest du erkunden?"
                        : " Ich bin hier, um deine Lernziele zu unterstützen. Wie kann ich dir helfen?",
                  },
                },
                pt: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? ` Vejo que você enviou "${titles}" - excelente!`
                      : ` Vejo que você enviou ${count} documentos: ${titles}. Ótimo!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " Vamos olhar juntos! O que você quer aprender?"
                        : " Estou muito animado para aprender com você! O que você gostaria de explorar?",
                    "3-5":
                      docTitles.length > 0
                        ? " Estou aqui para ajudá-lo a entender! Por onde começamos?"
                        : " Estou aqui para ajudá-lo a aprender! Qual assunto te interessa?",
                    "6-8":
                      docTitles.length > 0
                        ? " Estou pronto para ajudá-lo a dominar este material! Em que você quer trabalhar?"
                        : " Estou aqui para ajudá-lo a ter sucesso! Em qual assunto você quer focar?",
                    "9-12":
                      docTitles.length > 0
                        ? " Vamos explorar este material juntos. Quais conceitos você gostaria de aprofundar?"
                        : " Estou aqui para ajudá-lo a se destacar! Em qual tema você quer trabalhar?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " Estou pronto para ajudá-lo a analisar este material. Quais aspectos você gostaria de explorar?"
                        : " Estou aqui para apoiar seus objetivos de aprendizagem. Como posso ajudá-lo?",
                  },
                },
                zh: {
                  ...
                  closing: {
                    ...
                    "6-8":
                      docTitles.length > 0
                        ? "我准备好帮助你掌握这些内容了！你想做什么？"
                        : "我在这里帮助你成功！你想专注于哪个科目？",
                    "9-12":
                      docTitles.length > 0
                        ? "让我们一起探索这些内容。你想深入了解哪些概念？"
                        : "我在这里帮助你出类拔萃！你想学习什么主题？",
                    "College/Adult":
                      docTitles.length > 0
                        ? "我准备好帮助你分析这些内容了。你想探索哪些方面？"
                        : "我在这里支持你的学习目标。我能怎么帮助你？",
                  },
                },
                ja: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? `「${titles}」をアップロードしたのが見えます - 素晴らしい！`
                      : `${count}つのドキュメントをアップロードしたのが見えます：${titles}。いいですね！`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? "一緒に見てみましょう！何を学びたいですか？"
                        : "一緒に学べてとても嬉しいです！何を探求したいですか？",
                    "3-5":
                      docTitles.length > 0
                        ? "理解するのをお手伝いします！どこから始めましょうか？"
                        : "学習のお手伝いをします！どの科目に興味がありますか？",
                    "6-8":
                      docTitles.length > 0
                        ? "この教材をマスターするお手伝いをする準備ができています！何に取り組みたいですか？"
                        : "成功するお手伝いをします！どの科目に集中したいですか？",
                    "9-12":
                      docTitles.length > 0
                        ? "一緒にこの教材を探求しましょう。どの概念を深めたいですか？"
                        : "優秀になるお手伝いをします！どのトピックに取り組みたいですか？",
                    "College/Adult":
                      docTitles.length > 0
                        ? "この教材の分析をお手伝いする準備ができています。どの側面を探求したいですか？"
                        : "あなたの学習目標をサポートします。どのようにお手伝いできますか？",
                  },
                },
                ko: {
                  ...
                  docAck: (count, titles) =>
                    count === 1
                      ? `"${titles}"를 업로드하신 것을 보았습니다 - 훌륭합니다!`
                      : `${count}개의 문서를 업로드하신 것을 보았습니다: ${titles}. 좋아요!`,
                  closing: {
                    "K-2":
                      docTitles.length > 0
                        ? " 함께 살펴봐요! 무엇을 배우고 싶어요?"
                        : " 함께 배우게 되어 너무 기뻐요! 무엇을 탐험하고 싶어요?",
                    "3-5":
                      docTitles.length > 0
                        ? " 이해하는 것을 도와드릴게요! 어디서 시작할까요?"
                        : " 배우는 것을 도와드릴게요! 어떤 과목에 관심 있어요?",
                    "6-8":
                      docTitles.length > 0
                        ? " 이 자료를 마스터하는 것을 도와드릴 준비가 됐어요! 무엇을 공부하고 싶어요?"
                        : " 성공할 수 있도록 도와드릴게요! 어떤 과목에 집중하고 싶어요?",
                    "9-12":
                      docTitles.length > 0
                        ? " 함께 이 자료를 탐구해봐요. 어떤 개념을 깊이 이해하고 싶어요?"
                        : " 뛰어나게 되도록 도와드릴게요! 어떤 주제를 공부하고 싶어요?",
                    "College/Adult":
                      docTitles.length > 0
                        ? " 이 자료를 분석하는 것을 도와드릴 준비가 됐습니다. 어떤 측면을 탐구하고 싶으세요?"
                        : " 학습 목표를 지원해드릴게요. 어떻게 도와드릴까요?",
                  },
                },
              };
              ...
              const ageClosing =
                langGreeting.closing[ageGroup] ||
                langGreeting.closing["College/Adult"];

              if (docTitles.length > 0) {
                return (
                  langGreeting.intro +
                  langGreeting.docAck(docTitles.length, docTitles.join(", ")) +
                  ageClosing
                );
              } else {
            ...
            console.log(
              `[Custom Voice] 🌍 Generated greeting in language: ${state.language}`,
            );
            ...
            state.deepgramConnection = await startDeepgramStream(
              async (
                ...
                const spokenLang = detectedLanguage || state.language;
                console.log(
                  `[Deepgram] ${isFinal ? "✅ FINAL" : "⏳ interim"}: "${transcript}" (isFinal=${isFinal}, detectedLang=${spokenLang})`,
                );

                // CRITICAL FIX (Nov 14, 2025): Check userId FIRST to debug 401 auth issues
                if (!state.userId) {
                  console.error(
                    `[Deepgram] ❌ CRITICAL: userId missing in transcript handler!`,
                    {
                      sessionId: state.sessionId,
                  ...
                  console.error(
                    `[Deepgram] ❌ This means /api/user returned 401 and session initialization failed`,
                  );
                ...
                if (
                  ...
                  console.log(
                    `[Custom Voice] 🛑 BARGE-IN on ${isFinal ? "final" : "interim"} transcript: "${transcript}" (audio sent ${timeSinceLastAudio}ms ago)`,
                  );
                  ...
                  console.log(
                    "[Custom Voice] ✅ Barge-in processed, ready to listen to student",
                  );
                }
                ...
                if (state.isTutorSpeaking && timeSinceLastAudio >= 30000) {
                  console.log(
                    "[Custom Voice] ⏸️ Resetting stale tutor speaking state...",
                  );
                  state.isTutorSpeaking = false;
                }
                // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                ...
                if (!isFinal) {
                  ...
                  if (responseTimer) {
                    ...
                    responseTimer = null;
                    console.log(
                      "[Custom Voice] ⏱️ Timer cleared due to continued speech (interim)",
                    );
                  }
                  ...
                  state.interimFinalizeTimer = setTimeout(() => {
                    ...
                    console.log(
                      `[Custom Voice] ⏰ Interim fallback fired after ${interimDelay}ms; promoting to final: "${fallbackText}"`,
                    );
                  ...
                  console.log(
                    "[Custom Voice] ⏭️ Skipping interim for AI processing (barge-in already checked)",
                  );
                  return;
                }
                ...
                if (!transcript || transcript.trim().length < 3) {
                  console.log(
                    "[Custom Voice] ⏭️ Skipping short/empty transcript",
                  );
                  return;
                }
                ...
                console.log(
                  "[Inactivity] 🎤 User activity detected, timer reset",
                );
                ...
                // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                console.log(
                  `[Pipeline] 1. Transcript received: "${transcript}", isProcessing=${state.isProcessing}`,
                );
                ...
                if (spokenLang && spokenLang !== state.language) {
                  console.log(
                    `[Custom Voice] 🌍 Language switch detected: ${state.language} → ${spokenLang}`,
                  );
                ...
                console.log(
                  `[Custom Voice] ✅ Processing FINAL transcript: "${transcript}" (lang: ${state.detectedLanguage || state.language})`,
                );
                ...
                console.log(
                  `[Custom Voice] 📚 Accumulated transcript: "${fullTranscript}"`,
                );
                ...
                const delay = isIncomplete
                  ? TIMING_CONFIG.SERVER_DELAY_INCOMPLETE_THOUGHT
                  : TIMING_CONFIG.SERVER_DELAY_COMPLETE_THOUGHT;

                if (isIncomplete) {
                  console.log(
                    `[Custom Voice] ⏸️ Incomplete thought detected: "${fullTranscript}" - waiting ${delay}ms`,
                  );
                } else {
                  console.log(
                    `[Custom Voice] ⏰ Complete thought - waiting ${delay}ms before responding`,
                  );
                }
                ...
                responseTimer = setTimeout(() => {
                  console.log(
                    `[Custom Voice] ⏰ Processing after ${delay}ms pause`,
                  );
              ...
              async () => {
                ...
                if (!state.isSessionEnded && state.sessionId) {
                  console.warn(
                    "[Custom Voice] ⚠️ Unexpected Deepgram close while session active - attempting reconnect",
                  );
                  ...
                  if (reconnectAttempts > 3) {
                    console.error(
                      "[Custom Voice] ❌ Max reconnect attempts (3) reached, giving up",
                    );
                    ...
                    try {
                      ws.send(
                        JSON.stringify({
                          type: "error",
                          error:
                            "Voice connection lost. Please restart the tutoring session.",
                        }),
                      );
                    } catch (sendError) {
                      console.error(
                        "[Custom Voice] ❌ Failed to notify client:",
                        sendError,
                      );
                    }
                    return;
                  ...
                  console.log(
                    `[Custom Voice] 🔄 Reconnect attempt ${reconnectAttempts}/3...`,
                  );
                  ...
                  console.log(
                    `[Custom Voice] 🔄 Reconnecting in ${backoffDelay}ms (attempt ${reconnectAttempts}/3)...`,
                  );

                  setTimeout(async () => {
                    try {
                      console.log(
                        "[Custom Voice] 🔄 Creating new Deepgram connection...",
                      );
                      ...
                      console.log(
                        "[Custom Voice] ✅ Deepgram reconnected successfully",
                      );
                      ...
                      console.error(
                        "[Custom Voice] ❌ Reconnect attempt failed:",
                        reconnectError,
                      );
                        ...
                        try {
                          ws.send(
                            JSON.stringify({
                              type: "error",
                              error:
                                "Voice connection failed after multiple attempts. Please restart the session.",
                            }),
                          );
                        } catch (sendError) {
                          // Ignore
            ...
            try {
              const greetingAudio = await generateSpeech(
                greeting,
                state.ageGroup,
                state.speechSpeed,
              );
              ...
              console.log(
                `[Custom Voice] 🔊 Sent greeting audio (${greetingAudio.length} bytes)`,
              );
            } catch (error) {
              console.error(
                "[Custom Voice] ❌ Failed to generate greeting audio:",
                error,
              );
            }
          ...
          case "audio":
            ...
            console.log(
              "[Custom Voice] 📥 Audio message received from frontend:",
              {
            ...
            if (state.isReconnecting) {
              console.warn(
                "[Custom Voice] ⏸️ Audio dropped - reconnection in progress",
              );
              break;
            }
            ...
            if (state.deepgramConnection && message.data) {
              try {
                ...
                const int16View = new Int16Array(
                  ...
                  Math.min(10, audioBuffer.length / 2),
                );
                const hasNonZero = int16View.some((sample) => sample !== 0);
                const maxAmplitude = Math.max(
                  ...Array.from(int16View).map(Math.abs),
                );
                ...
                if (!hasNonZero) {
                  console.warn(
                    "[Custom Voice] ⚠️ Audio buffer is COMPLETELY SILENT (all zeros)!",
                  );
                }
                ...
                console.log(
                  "[Custom Voice] ✅ Audio forwarded to Deepgram successfully",
                );
              } catch (error) {
                console.error(
                  "[Custom Voice] ❌ Error sending audio to Deepgram:",
                  {
                    error:
              ...
              console.error("[Custom Voice] ❌ Cannot forward audio:", {
                ...
                hasData: !!message.data,
                reason: !state.deepgramConnection
          ...
          case "text_message":
            ...
            console.log(
              `[Custom Voice] 📝 Text message from ${state.studentName}: ${message.message}`,
            );
            ...
            state.inactivityWarningSent = false; // Reset warning flag
            console.log(
              "[Inactivity] ⌨️ User text activity detected, timer reset",
            );
            ...
            try {
              ...
              if (!moderation.isAppropriate) {
                console.warn(
                  "[Custom Voice] ⚠️ Inappropriate text content detected",
                );
                ...
                const warningAudio = await generateSpeech(
                  warningText,
                  state.ageGroup,
                  state.speechSpeed,
                );
              ...
              await new Promise<void>((textResolve, textReject) => {
                const textCallbacks: StreamingCallbacks = {
                  onSentence: async (sentence: string) => {
                    textSentenceCount++;
                    console.log(
                      `[Custom Voice] 📤 Text sentence ${textSentenceCount}: "${sentence.substring(0, 50)}..."`,
                    );
                    ...
                    if (state.tutorAudioEnabled) {
                      try {
                        ...
                        console.log(
                          `[Custom Voice] 🔊 Text sentence ${textSentenceCount} TTS: ${audioBuffer.length} bytes`,
                        );
                        ...
                        console.error(
                          `[Custom Voice] ❌ Text TTS error for sentence ${textSentenceCount}:`,
                          ttsError,
                        );
                      }
                    }
                  ...
                  onComplete: (fullText: string) => {
                    ...
                    console.log(
                      `[Custom Voice] ⏱️ Text streaming complete: ${textStreamMs}ms, ${textSentenceCount} sentences`,
                    );
                    console.log(
                      `[Custom Voice] 🤖 Tutor (text): "${fullText}"`,
                    );
                  ...
                  onError: (error: Error) => {
                    console.error(
                      "[Custom Voice] ❌ Text streaming error:",
                      error,
                    );
                    textReject(error);
                  },
              ...
              console.log(
                `[Custom Voice] 🔊 Sent streamed tutor voice response (${textSentenceCount} chunks)`,
              );
              ...
              console.error(
                "[Custom Voice] Error processing text message:",
                error,
              );
            }
            break;
          ...
          case "document_uploaded":
            ...
            console.log(
              `[Custom Voice] 📄 Document uploaded during session: ${message.filename}`,
            );
            ...
            try {
              ...
              if (document) {
                ...
                if (chunks && chunks.length > 0) {
                  ...
                  console.log(
                    `[Custom Voice] ✅ Added document to session context (${chunks.length} chunks)`,
                  );
                  ...
                  const ackAudio = await generateSpeech(
                    ackMessage,
                    state.ageGroup,
                    state.speechSpeed,
                  );
                  ...
                  console.error(
                    `[Custom Voice] Document has no chunks: ${message.documentId}`,
                  );
                }
              } else {
                console.error(
                  `[Custom Voice] Document not found: ${message.documentId}`,
                );
              }
            } catch (error) {
              console.error(
                "[Custom Voice] Error adding document to session:",
                error,
              );
            }
            break;
          ...
          case "speech_detected":
            ...
            if (state.isTutorSpeaking && timeSinceAudioForVAD < 30000) {
              console.log(
                `[Custom Voice] 🛑 BARGE-IN via client VAD (audio sent ${timeSinceAudioForVAD}ms ago)`,
              );
              ...
              console.log(
                `[Custom Voice] ℹ️ speech_detected received (tutor not speaking, syncing state)`,
              );
              state.isTutorSpeaking = false;
            }
            break;
          ...
          case "end":
            ...
            console.log(
              "[Session End] Transcript length:",
              state.transcript.length,
            );
            console.log(
              "[Session End] Session already ended?",
              state.isSessionEnded,
            );
            console.log(
              "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
            ...
            try {
              await finalizeSession(state, "normal");
              console.log(
                "[Session End] ✅ finalizeSession completed successfully",
              );
            } catch (error) {
            ...
            console.log(
              "[Session End] 📤 Sending session_ended ACK to client...",
            );
            ws.send(
              JSON.stringify({
                type: "session_ended",
          ...
          default:
            console.warn(
              "[Custom Voice] ⚠️ Unknown message type:",
              message.type,
            );
        }
      } catch (error) {
    ...
    ws.on("close", async () => {
      ...
      if (state.isSessionEnded) {
        console.log(
          "[Custom Voice] ℹ️ Session already finalized, skipping close handler",
        );
        return;
      }
    ...
    ws.on("error", async (error) => {
      ...
      if (state.isSessionEnded) {
        console.log(
          "[Custom Voice] ℹ️ Session already finalized, skipping error handler",
        );
        return;
      }
