...
-   **Comprehensive Timing Fix** (Nov 4, 2025): Complete overhaul of voice timing to eliminate student answer cutoffs. Deepgram endpointing increased to 3500ms (was 2000ms), TTS default speed reduced to 0.85 (was 0.95) for slower, clearer speech. Server-side adaptive delays: 1200ms for complete thoughts, 2500ms for incomplete fragments ("um", "I think"). Post-interruption buffer adds 2500ms extra delay when student interrupts tutor. Incomplete thought detection identifies short responses and trailing conjunctions. **Total delay: 4.7-8.5 seconds from student stops speaking → tutor responds** (Deepgram 3500ms + server 1200-2500ms + post-interrupt buffer 2500ms when applicable). Achieves natural conversation flow without premature cutoffs.
-   **Document Upload Reliability** (Nov 3, 2025): Added 30-second timeout to PDF text extraction to prevent indefinite hangs on complex documents. Failed uploads are now completely deleted from database and disk instead of being marked as "failed", eliminating duplicate/clutter entries in document lists. Comprehensive error logging tracks extraction failures for debugging.
-   **User-Controlled Speech Speed** (Oct 31, 2025): Dashboard Speech Speed slider (0.7-1.2 range per ElevenLabs API limits) directly controls voice pace in tutoring sessions. Default reduced to 0.85 (Nov 4, 2025) for better comprehension. Includes NaN validation, range clamping, and graceful fallback to default for invalid values. Backend automatically clamps all speed values (including legacy out-of-range data) to ElevenLabs' required 0.7-1.2 range with comprehensive logging for debugging.
...
-   **Flexible Communication Modes** (Nov 4, 2025): Three distinct communication modes for different learning environments and preferences: **Voice Mode** (full audio conversation with microphone and tutor audio), **Hybrid Mode** (listen-only with microphone off, students can hear tutor but respond via text), and **Text-Only Mode** (complete silence with all audio muted, ideal for libraries, quiet environments, or late-night studying). Mode preferences persist in localStorage, quick toggle buttons enable instant mode switching during active sessions, server-side conditional audio generation skips TTS processing when tutor audio is muted, and mode changes create system messages in transcript for session context. All modes support full document processing, file uploads, and AI tutoring capabilities.
