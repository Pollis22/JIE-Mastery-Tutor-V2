...
The RAG system supports various document formats (PDF, DOCX, Images via OCR, XLSX, TXT, XML). The processing pipeline involves upload, text extraction, chunking, embedding (OpenAI text-embedding-3-small), and storage in `pgvector`. OCR supports 25 languages. Document activation uses a two-tier model: documents are "uploaded" (exist in the document list) vs "Active" (explicitly checkbox-selected for the session). Only Active documents are injected into the LLM prompt via RAG — inactive documents are never used for retrieval or mentioned by the tutor. The `DOCS_FALLBACK_TO_ALL_IF_NONE_ACTIVE` flag defaults to `false` (opt-in only). The `session-docs-service.ts` manages per-session activation state. The tutor greeting acknowledges Active documents once (capped at 3 filenames), and a `DOCUMENT ACKNOWLEDGMENT RULE` in the system prompt prevents re-listing document names in subsequent turns. The `hasAcknowledgedDocs` and `uploadedDocCount` fields on SessionState support the one-time greeting logic. The frontend sends `uploadedDocCount` (total uploaded docs) via the WebSocket init message for greeting context.

### Continuity Memory System
A per-student memory system provides cross-session context for personalized tutoring. Upon session conclusion, a background job generates a structured summary using Claude, which includes topics covered, concepts mastered/struggled, and student insights. At session start, the last 5 summaries are injected into the Claude prompt. The system uses a DB-based job queue with retry logic and is triggered via `/api/cron/memory-jobs`. All memory operations are non-blocking with safe fallbacks. Returning students receive adaptive continuity greetings, and a "First-Turn-Only Guarantee" prevents duplicate greetings on WebSocket reconnect. Student isolation ensures no cross-topic leakage between siblings. The `studentId` is propagated from the frontend through the WebSocket init message to the backend session state, enabling strict per-student filtering in `getRecentSessionSummaries()`. Profile switches force-end active voice sessions before switching to prevent mid-session contamination. A regression test script (`scripts/test-student-isolation.ts`) validates student isolation using real DB data. Observability: `[CONTINUITY]` log prefix tracks memory load operations including studentId resolution source.
